...
> **Mô hình kiến trúc triển khai:** 4 cấp độ kỹ thuật chuẩn mực:  
...
> 
> **Nguyên tắc cốt lõi cho AI Coding Agent:**  
> - **Scope rõ ràng:** Mỗi Task trả lời chính xác: Làm gì, Tại sao cần, Trong phạm vi nào, Tuyệt đối không làm gì.  
> - **Dependency thực tế:** Phân định rạch ròi giữa Hard Dependency (bắt buộc tuần tự) và Parallel Execution (có thể chạy độc lập).  
> - **Tiêu chí nghiệm thu định lượng:** Đánh giá dựa trên bằng chứng vật lý (Tests pass, Flyway validated, HTTP status đúng), không suy đoán cảm tính.  
> - **Context nhỏ & Khép kín:** Một Task là một lát cắt thay đổi có ý nghĩa (Coherent Change), không gom mega-task và không băm vụn vi mô.  
...
Phase 1: Spring Boot Foundation & Web Infrastructure [IN_PROGRESS: Mod 1A Done, Mod 1B Pending]
   ...
   ├──────────────────────────────────────┐
   ▼                                      ▼ (Song song nếu cần)
Phase 2: Persistence Layer & Seed Data    Phase 1 Module 1B: Response Envelope
(JPA Mappings 2A-2D, Seed 2E, Test 2F)    (ApiResponse, GlobalExceptionHandler)
   │                                      │
   └──────────────────┬───────────────────┘
                      ▼
Phase 3: Authentication, Security & RBAC (Security 6, JWT, Login/Register, Profile)
   │
   ▼
...
Phase 9: Frontend UI & Client Integration (Layout, api.js, Learner, SRS, Creator, Moderator UI)
   │
   ▼
Phase 10: Security Hardening & Performance QA (OWASP Audit, Rate Limiting, Index Explain, Coverage)
   │
   ▼
...
- **Mục tiêu:** Thiết lập lại toàn bộ nền tảng tài liệu đặc tả thẩm quyền, dọn dẹp mã nguồn cũ và cấu hình hạ tầng CSDL cục bộ.
- **Trạng thái:** **`COMPLETED`** (Đã nghiệm thu với đầy đủ bằng chứng vật lý).

#### Module 0A: Project Reset & Agent Skills Protection
* **Task 0A.1:** Xóa bỏ 100% mã nguồn và artifacts cũ trong `backend/`, `frontend/`, `scripts/`, `temp/`, `docs/`.
...
* **Task 1B.1: Base Response Models (`ApiResponse<T>`, `PageResponse<T>`, `ErrorCode` enum)**
  - *Mục đích:* Chuẩn hóa 100% định dạng JSON trả về client theo Mục 1.2 và 1.3 của `API.md`.
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào `Module 1A` (Flyway V1 schema). **KHÔNG phụ thuộc vào `Module 1B`** (có thể triển khai độc lập hoặc song song).
- **Trạng thái:** **`NOT_STARTED`**.

#### Module 2A: Identity & Role Persistence Mapping
...
#### Module 2B: Dictionary Catalog Persistence Mapping
* **Task 2B.1: JPA Entities Cụm Từ điển (`RADICAL`, `VOCABULARY`, `VOCAB_RADICAL`)**
  - *Phạm vi:* Entity cho `Radical`, `Vocabulary` (lưu cả `pinyin` có dấu và `pinyin_raw` không dấu), ánh xạ liên kết N:N `VOCAB_RADICAL`.
  - *depends_on:* `Task 1A.4`.
...
* **Task 2C.1: JPA Entities Cụm Bài học (`LESSON`, `LESSON_VOCABULARY`)**
  ...
  - *depends_on:* `Task 2A.1`, `Task 2B.1`.
...
* **Task 2D.1: JPA Entities Cụm Ghi chú & Kiểm toán (`USER_SRS_SETTING`, `PERSONAL_NOTE`, `MODERATION_LOG`)**
  ...
  - *depends_on:* `Task 2A.1`, `Task 2B.1`, `Task 2C.1`.
...
#### Module 2E: Database Seed Migrations (Flyway)
...
* **Task 2F.1: Kiểm thử tích hợp toàn diện tầng Persistence & Schema Validation**
  - *Phạm vi:* Bật cấu hình Hibernate `spring.jpa.hibernate.ddl-auto: validate`, chạy toàn bộ test suite để đảm bảo 14 Entity mappings khớp 100% với schema MySQL do Flyway quản lý.
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào `Module 1B` (`ApiResponse` / Exception Handling), `Module 2A` (Account/Role persistence) và `Module 2E` (Seed roles).
- **Trạng thái:** **`NOT_STARTED`**.

#### Module 3A: Spring Security & JWT Infrastructure
* **Task 3A.1:** Cấu hình `SecurityConfig`: Spring Security 6 `SecurityFilterChain`, `BCryptPasswordEncoder`, cấu hình session `STATELESS`, vô hiệu hóa CSRF cho REST.
...
* **Task 3B.2:** `AuthService` và `AuthController` (`POST /api/v1/auth/register` gán mặc định role `Learner`, `POST /api/v1/auth/login` kiểm tra mật khẩu qua BCrypt, trả về JWT).
...
* **Task 3C.1:** DTOs (`UserProfileResponse`, `UpdateProfileRequest`), `UserProfileService` và `UserProfileController` (`GET /api/v1/users/profile`, `PUT /api/v1/users/profile` lấy và cập nhật profile của user đang đăng nhập).
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào `Module 1B`, `Module 2B`, `Module 2E`, và `Phase 3` (xác thực quyền Admin).
- **Trạng thái:** **`NOT_STARTED`**.
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào `Module 1B`, `Module 2C`, `Phase 3` (Creator auth), `Phase 4` (Vocabulary catalog).
- **Trạng thái:** **`NOT_STARTED`**.
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào `Module 1B`, `Module 2C`, `Module 2D`, `Phase 3` (Moderator auth), `Phase 5` (Pending lessons).
- **Trạng thái:** **`NOT_STARTED`**.
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào `Module 1B`, `Module 2D`, `Phase 3` (Learner auth), `Phase 4` & `5` (Vocabulary/Radical items).
- **Trạng thái:** **`NOT_STARTED`**.
...
* **Task 7A.2:** Unit test toán học toàn diện cho `SrsCalculator` với độ phủ 100% các nhánh rẽ và điều kiện biên.
* **Checkpoint 7A:** Unit test độc lập toán học đạt 100% pass, xác nhận tính toán chính xác tuyệt đối không phụ thuộc database hay Spring context.
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào `Module 1B`, `Module 2D`, `Phase 3` (Learner auth), `Phase 4` (Vocabulary).
- **Trạng thái:** **`NOT_STARTED`**.
...
- **Ranh giới phụ thuộc:** Phụ thuộc vào toàn bộ backend APIs (`Phase 3` $\rightarrow$ `Phase 8`).
- **Trạng thái:** **`NOT_STARTED`**.
...
### Phase 10 — Security Hardening, Performance & Quality Assurance
- **Mục tiêu:** Rà soát an ninh ứng dụng theo chuẩn OWASP, gia cố chống tấn công brute-force, tối ưu hóa hiệu năng truy vấn CSDL và nâng cao độ phủ kiểm thử tự động.
- **Ranh giới:** Đây là giai đoạn gia cố an ninh và lấp đầy khoảng trống kiểm thử (Coverage Gaps), không phải nơi bắt đầu viết test lần đầu.
- **Trạng thái:** **`NOT_STARTED`**.

#### Module 10A: Application Security Hardening
* **Task 10A.1:** Rà soát an ninh OWASP toàn diện: cấu hình CORS chặt chẽ, kiểm tra chống XSS DOM, rà soát JPA parameterized query (chống SQL Injection 100%), kiểm tra an toàn upload file Excel (MIME type, max size 10MB, chống path traversal).
* **Task 10A.2:** Cơ chế kiểm soát tần suất gọi (Rate Limiting) trên Auth endpoints (`/api/v1/auth/login`) chống tấn công vét cạn dò mật khẩu.
* **Checkpoint 10A:** Bộ test bảo mật: gửi payload XSS/SQLi bị xử lý an toàn; gửi 20 request login liên tiếp kích hoạt HTTP 429 Too Many Requests.

#### Module 10B: Database Performance & Coverage Gap Closure
* **Task 10B.1:** Tối ưu hóa truy vấn CSDL & Xác minh chỉ mục MySQL: chạy `EXPLAIN` trên các truy vấn trọng yếu (`idx_card_progress_due`, `idx_vocab_pinyin_raw`, `idx_lesson_status`), đảm bảo không có table scan trên bảng lớn.
* **Task 10B.2:** Lấp đầy khoảng trống kiểm thử tự động: bổ sung test case cho các điều kiện biên của Service và Controller còn thiếu.
* **Checkpoint Phase 10:** Toàn bộ test suite chạy thành công (`mvn clean verify`), không có cảnh báo bảo mật nghiêm trọng.
...
* **Checkpoint 11A:** Cả 2 kịch bản E2E tự động chạy thành công 100% không phát sinh lỗi.

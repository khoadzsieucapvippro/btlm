...
| **Phase 1** | **Mod 1B** | **Task 1B.1** | Xây dựng Base Response DTOs (`ApiResponse<T>`, `PageResponse<T>`, `ErrorCode`) | **COMPLETED** | Task 1A.4 | `ApiResponseTests` PASS 12/12 tests (0.35s); `ApiResponse<T>`, `PageResponse<T>`, `ErrorCode` tạo đúng spec API.md; `mvn clean test` PASS 13/13 tests. |

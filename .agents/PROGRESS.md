...
| **Phase 1** | **Mod 1B** | **Task 1B.2** | Xây dựng `GlobalExceptionHandler` (`@RestControllerAdvice`) | **COMPLETED** | Task 1B.1 | `GlobalExceptionHandlerTests` PASS 12/12 tests (0.33s); bắt lỗi Validation (400), BusinessException (dynamic status), generic fallback (500); `mvn clean test` PASS 25/25 tests. |
